;+
; NAME:		READ_HK_FILES
; PURPOSE:	Read housekeeping files associated with image files
; CALLING SEQUENCE:
;	success = read_hk_files(data_dir, file, hdr, hk_out, att_out)
; INPUTS:
...
;Define structure to hold important variables from HK file and from attitude
;file
    hk = {hk, time:0., filter:0., cath_volt:0., anode_volt:0., mcp_volt:0.}
    att = {att, time:0., roll_ra:0d, roll_dec:0d, roll_rot:0d}

;We should know the detector to read the right keywords
	detector = strcompress(sxpar(data_hdr, "detector"),/remove)

;There are many HK files associated with one observation which we have to read
;and associate by time.
...
;If no file exists then return error
        jude_err_process,"errors.txt","Housekeeping file not found"
        return,exit_failure
    endif else begin

;Read HK files if they exist    
...
;There are many different HK files associated with a single observation so I 
;merge them here.
			...
			endif else begin	;End FUV Block
				if (n_elements(hk_out) eq 0) then begin
